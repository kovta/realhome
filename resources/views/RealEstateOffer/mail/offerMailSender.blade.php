@php
    /**
    * @var \App\Models\OfferMailSenderData $record
    */
@endphp
...
@section('content')
    ...
    <div class="admin-form-wrapper">
        ...
        <form method="POST" action="{{route('realEstateOffers.sendmail', [$record->offerId])}}" class="col-xl-6 col-lg-12">
            @csrf
            @method('POST')
...
{{--                            <input type="hidden" name="cc" id="cc" value="{{$record->sender}}">--}}
                            <label class="form-check-label" for="ccCheck">
                                @lang('messages.sendmail_cc'){{-- {{$record->sender}}--}}
                            </label>
                        </div>
                    </div>
                </div>
