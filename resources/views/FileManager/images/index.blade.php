    ...
    <script type="text/javascript">
        ...
        class FileManager {
            ...
            loadItems() {
                ...
                $('#itemContainer').load('{{ route('filemanagerItems')  }}?fmKey=' + fileManager.fmKey, function () {

                    var sortable = Sortable.create( document.getElementById('sortableRows') , {
                        onEnd: function (event) {
                            var itemEl = event.item;  // dragged HTMLElement
                            var id = $(itemEl).attr('data-id');
                            $.post('{{ route('filemanagerDragReorder') }}', { 'fmKey': fileManager.fmKey, id: id, oldIndex: event.oldIndex, newIndex: event.newIndex } )
                                ...
                                .always(function () {
                                    if (event.oldIndex != event.newIndex) fileManager.loadItems();
                                });
                        },
                    });
...
@section('content')
    ...
    <div class="admin-form-wrapper">
        <div class="admin-form-title">{{ $fileManager->ownerName }}'s images</div>

        <div class="row">
            <div class="col-lg-12" style="padding: 20px;">
                <a href="{{ route('realEstates.edit', request()->entityId) }}" class="btn btn-primary">@lang('messages.btn_back')</a>
            </div>
        </div>
                    ...
                    <form id="FileManagerUploadForm" class="form-inline" method="POST" action="{{ route('filemanagerUpload') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group fileupload">
                                <input id="dokumentumNev" name="dokumentumNev[]" type="file" multiple="multiple" onchange="$('#dokumentumNevInput').val($(this).val().replace('C:\\fakepath\\', ''));" />
                        ...
                        <button class="btn btn-secondary" type="submit" style="margin: 0 10px;">@lang('messages.filemanager_upload_caption')</button>
                        {{--
                        ...
                        --}}
                        <button id="reorderButton" class="btn btn-secondary" style="margin: 0 10px;">@lang('messages.filemanager_reorder_caption')</button>
                    </form>
                </div>
            </div>

            <div id="itemContainer" class="row filemanager-item-container">
            </div>
<!--
